...
class SongListViewController: BaseViewController {
    ...
    // 初期生成の場合に、songArrayが0件のため、用いる <- ここは設計変えたほうがいいかもしれないですね
    // folder生成した瞬間にもーまとめて最初として、生成しちゃうなどそうすれば、いらないかな
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "ComposeSong" {
            ...
            let lyricA = Lyric()
            // AでユニークID発行し、以降1ずつインクリメント
            lastId = self.newId(model: lyricA)!
            
            lyricA.owner = song
            lyricA.id = lastId
            lyricA.name = "Aメロ"
            lyricA.text = "これは曲追加時にデフォルトで設定する歌詞ですA"
            let lyricB = Lyric()
            lyricB.owner = song
            lyricB.id = lastId + 1
            lyricB.name = "Bメロ"
